...
import { Bot, ChevronRight, ExternalLink, Send, Sparkles, User2, X } from "lucide-react";
...
export default function MindChatPage() {
  ...
  const [activeCrisisResources, setActiveCrisisResources] = useState<
    Array<{
      lineName: string;
      phoneNumber: string;
      website: string;
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      if (response.data.showCrisisResources && response.data.crisisResources?.length) {
        setActiveCrisisResources(response.data.crisisResources);
      }
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-4 py-5 md:px-7 md:py-7">
        {isBootstrapping ? (
          ...
          <div className="space-y-4">
            ...
            {activeCrisisResources.length > 0 && (
              <div className="rounded-[28px] border border-[#FF7A6E]/30 bg-gradient-to-br from-[#321717] via-[#251111] to-[#1A0B0B] p-4 shadow-[0_18px_50px_rgba(0,0,0,0.3)] md:p-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-[#FFB3A8]">
                  ...
                  <button
                    type="button"
                    onClick={() => setActiveCrisisResources([])}
                    className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white/70 transition-colors hover:border-white/20 hover:bg-white/[0.08] hover:text-white"
                ...
                <div className="mt-4 space-y-3">
                  {activeCrisisResources.map((resource) => (
                    <div
                      key={`${resource.lineName}-${resource.phoneNumber}-${resource.country}`}
                      className="rounded-[20px] border border-white/10 bg-white/[0.04] px-4 py-4"
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="min-w-0 space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            <p className="font-semibold text-white">{resource.lineName}</p>
                          ...
                          <p className="text-xs uppercase tracking-[0.18em] text-white/45">{resource.country}</p>

                          {resource.phoneNumber && (
                            <a
                              href={`tel:${resource.phoneNumber}`}
                              className="block text-sm font-medium text-[#FFB3A8] underline decoration-[#FFB3A8]/40 underline-offset-4 transition-colors hover:text-white"
                            >
                              {resource.phoneNumber}
                            </a>
                          )}
                        ...
                        {resource.website && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 rounded-full border border-[#FFB3A8]/20 bg-[#FFB3A8]/10 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#FFD2CB] transition-colors hover:border-[#FFB3A8]/35 hover:bg-[#FFB3A8]/15"
                          >
                            Website
